  ...
  return (
    <div className="max-w-[800px] mx-auto px-6 py-8">
      <div className="card p-8 md:p-10">
        <h1 className="text-2xl font-semibold mb-8">Update Listing</h1>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          {existingImages.length > 0 && (
            <div>
              <label className="block text-sm font-medium mb-3">Existing Images</label>
              <div className="flex gap-3 overflow-x-auto pb-2">
                {existingImages.map((img, i) => (
                  <div key={i} className="w-24 h-24 rounded-xl overflow-hidden flex-shrink-0 border border-grey-light">
                    <img src={`http://localhost:5000${img}`} alt={`Existing ${i}`} className="w-full h-full object-cover" />
                  </div>
                ))}
              </div>
            </div>
          ...
          <div>
            <label className="block text-sm font-medium mb-1.5">Add New Images</label>
